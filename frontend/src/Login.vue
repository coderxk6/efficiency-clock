...
const handleGuestLogin = async () => {
  ...
  try {
    ...
    // 如果没有游客专用 token，检查当前 token 是否是游客的
    if (!existingToken) {
      existingToken = localStorage.getItem('token');
      const existingIsGuest = localStorage.getItem('isGuest') === 'true';
      if (existingToken && existingIsGuest) {
        existingUsername = localStorage.getItem('username');
        existingNickname = localStorage.getItem('nickname');
      } else {
        existingToken = null;
      }
    }
    ...
    if (existingToken) {
      // 验证现有 token 是否有效（使用 /api/focus/tasks 接口）
      const verifyRes = await fetch('/api/focus/tasks', {
        headers: {
      ...
      if (verifyRes.ok) {
        // Token 有效，直接使用现有游客账号
        console.log('找到有效的游客 token，继续使用');
        localStorage.setItem('token', existingToken);
        localStorage.setItem('username', existingUsername);
        localStorage.setItem('nickname', existingNickname);
    ...
    // 创建新游客账号
    console.log('调用 /api/auth/guest 创建新游客账号');
