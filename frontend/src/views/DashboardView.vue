...
const loadRunningTasks = async () => {
  try {
    const body = await request.get('/api/focus/tasks');
    const data = body.code === 200 ? body.data : body; 
    const taskList = Array.isArray(data) ? data : (Array.isArray(body) ? body : []);
    
    runningTasks.value = taskList.map(task => ({
      ...task,
...
const loadCompletedTasks = async () => {
  try {
    const body = await request.get('/api/focus/history?limit=50');
    const data = body.code === 200 ? body.data : body;
    completedTasks.value = Array.isArray(data) ? data : (Array.isArray(body) ? body : []);
  } catch (err) {
...
const startFocus = async () => {
  ...
  try {
    const res = await request.post('/api/focus/start', {
    ...
    showToast('心魔干扰: ' + (err.message || '启动失败'));
  }
};
...
const completeTask = async (task) => {
  ...
  try {
    const res = await request.put(`/api/focus/${task.id}/complete`);
    const data = res.code === 200 ? res.data : res;

    userStore.updateCultivation(data.newRank, data.totalExperience);
    ...
    if (isStillRunning) {
      showToast('心魔干扰: ' + (err.message || '完成失败'));
    }
  }
};
...
const giveUpTask = (task) => {
  showDialog({
    ...
    try {
      await request.delete(`/api/focus/${task.id}`);
    } catch (err) {
...
const fireConfetti = () => {
  ...
  const interval = setInterval(function() {
    ...
    if (Math.random() < 0.05) {
       confetti({ 
         ...defaults, particleCount: 100, spread: 120, origin: { y: 0.6 },
         scalar: 1.2, colors: ['#7232dd', '#3fecff', '#ffe4e6']
       });
    }
  }, 200);
};
    ...
    <div class="header">
      ...
      <div class="status-card">
        ...
        <div class="cultivation-info">
          ...
          <div class="exp-text">灵力: {{ userStore.exp }}</div>
        </div>
      </div>
    </div>
...
<style>
/* Vant Dialog 全级深度定制 - 无分割线版 */
.van-dialog {
  background: #111019 !important; /* 极简深黑色 */
  border: 1px solid rgba(167, 139, 250, 0.15) !important; /* 带有微弱紫光的呼吸边框 */
...
.van-dialog__message {
  color: #94a3b8 !important; /* 高级银灰色，拒绝紫色 */
...
.van-dialog__confirm:hover {
  ...
  box-shadow: 0 12px 25px rgba(79, 172, 254, 0.4) !important;
}
