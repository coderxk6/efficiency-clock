...
const handleLoginSuccess = async (authData) => {
  console.log('登录成功:', authData.username);
  
  // 检查是否是同一用户重新登录
  const isSameUser = currentUser.value.username === authData.username;
  ...
  if (!isSameUser) {
    ...
    console.log('同一用户重新登录，保留现有数据');
  }
};

// 登出
const handleLogout = () => {
  const isGuest = currentUser.value.isGuest;
...
const loadRunningTasks = async () => {
  try {
    ...
    if (res.ok) {
      ...
      console.log('Loaded running tasks:', tasks);
      ...
      console.error('Failed to load running tasks:', res.status, error);
    }
  } catch (err) {
